...
from app.core.exceptions import ValidationError, DatabaseError, AIServiceError
...
async def create_ticket(
    ...
    validated_body = getattr(request.state, "validated_body", None)
    if not validated_body:
        raise ValidationError("Missing or invalid request body")
        
    # Parse validated body into Pydantic model
    try:
        ticket = TicketCreate(**validated_body)
    except Exception as e:
        raise ValidationError(f"Invalid ticket data: {str(e)}")
    try:
        # Validate input data
        if not ticket.subject.strip():
            raise ValidationError("Ticket subject cannot be empty", field="subject")
        if not ticket.description.strip():
            raise ValidationError("Ticket description cannot be empty", field="description")
        if ticket.user_email and not "@" in ticket.user_email:
        ...
        try:
            db_ticket = Ticket(
                ...
                user_email=ticket.user_email.lower().strip() if ticket.user_email else None,
            ...
            initial_interaction = TicketInteraction(
                ...
                sender=ticket.user_email or "Anonymous"
            )
            ...
            db.rollback()
            raise DatabaseError("Failed to create ticket in database", details={"error": str(e)})
        ...
        try:
            if ai_service.is_available():
                ...
                logger.warning("AI service unavailable - skipping AI processing")
        except Exception as e:
            logger.error(f"AI service error: {e}")
            raise AIServiceError("Failed to initialize AI processing", service="Claude")
    ...
    except (ValidationError, DatabaseError, AIServiceError) as e:
        # These exceptions will be handled by the global exception handler
        raise
    except Exception as e:
        logger.error(f"Unexpected error creating ticket: {e}")
        raise DatabaseError("Unexpected error creating ticket", details={"error": str(e)})
